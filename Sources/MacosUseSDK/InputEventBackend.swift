    ...
    @unchecked Sendable
{
    ...
    func prepare(_ event: InputEvent) throws -> PreparedInputEvent {
        ...
        guard let cgEvent else {
            throw MacosUseSDKError.inputSimulationFailed("failed to prepare input event")
        }
        let sourceIdentity = InputEventSourceIdentity(
    ...
    func cursorPosition() throws -> CGPoint {
        guard let event = CGEvent(source: nil) else {
            throw MacosUseSDKError.inputSimulationFailed("failed to read cursor position")
        }
    ...
    private func setUnicodeText(_ text: String, on event: CGEvent?) throws {
        guard let event else {
            throw MacosUseSDKError.inputSimulationFailed("failed to prepare Unicode keyboard event")
        }
