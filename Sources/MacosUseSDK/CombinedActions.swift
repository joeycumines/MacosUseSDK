...
public struct AttributeChangeDetail: Codable, Sendable {
    ...
    init<T: CustomStringConvertible>(attribute: String, before: T?, after: T?) {
        guard attribute != "text" else {
            ...
            oldValue = before.map(\.description)
            newValue = after.map(\.description)
            addedText = nil
            removedText = nil
            return
        }
        ...
        oldValue = before.map(\.description)
        newValue = after.map(\.description)
        addedText = nil // Not applicable
        removedText = nil // Not applicable
    }

    /// Initializer for Doubles (position/size)
    init(attribute: String, before: Double?, after: Double?, format: String = "%.1f") {
        attributeName = attribute
        oldValue = before.map { String(format: format, $0) }
        newValue = after.map { String(format: format, $0) }
        addedText = nil
        removedText = nil
    }
    ...
    init(textBefore: String?, textAfter: String?) {
        ...
        addedText = addedItems.isEmpty ? nil : String(addedItems.map(\.char))
        removedText = removedItems.isEmpty ? nil : String(removedItems.map(\.char))
