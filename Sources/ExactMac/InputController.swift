...
public extension ExactMacError {
    /// Add specific error cases relevant to InputController
    static func inputInvalidArgument(_ message: String) -> ExactMacError {
        .internalError("Input Argument Error: \(message)") // Reuse internalError or create specific types
    }

    static func inputSimulationFailed(_ message: String) -> ExactMacError {
...
/// - Throws: `ExactMacError` if the event source cannot be created or the event cannot be posted.
...
func clickMouse(
    ...
    guard point.x.isFinite, point.y.isFinite else {
        throw ExactMacError.inputInvalidArgument("click coordinates must be finite")
    }
...
func moveMouse(
    ...
    guard point.x.isFinite, point.y.isFinite else {
        throw ExactMacError.inputInvalidArgument("move coordinates must be finite")
    }
    ...
    guard start.x.isFinite, start.y.isFinite else {
        throw ExactMacError.inputSimulationFailed("cursor position is not finite")
    }
    let steps = 20
...
private func scrollDelta(_ value: Double, field: String) throws -> Int32 {
    guard value.isFinite else {
        throw ExactMacError.inputInvalidArgument("\(field) must be finite")
    }
    let rounded = value.rounded(.toNearestOrAwayFromZero)
    guard rounded >= Double(Int32.min), rounded <= Double(Int32.max) else {
        throw ExactMacError.inputInvalidArgument("\(field) is outside the supported range")
    }
...
func scrollMouse(
    ...
    if let point, !point.x.isFinite || !point.y.isFinite {
        throw ExactMacError.inputInvalidArgument("scroll coordinates must be finite")
    }
    ...
    guard horizontalDelta != 0 || verticalDelta != 0 else {
        throw ExactMacError.inputInvalidArgument("scroll must have a non-zero delta")
    }
...
func hoverMouse(
    ...
    guard hoverDuration > 0 else {
        throw ExactMacError.inputInvalidArgument("hover duration must be greater than zero")
    }
    guard point.x.isFinite, point.y.isFinite else {
        throw ExactMacError.inputInvalidArgument("hover coordinates must be finite")
    }
    try backend.checkPostAccess()
...
/// - Throws: `ExactMacError` if any event cannot be created or posted.
...
func performDrag(
    ...
    guard path.allSatisfy({ $0.x.isFinite && $0.y.isFinite }) else {
        throw ExactMacError.inputInvalidArgument("drag coordinates must be finite")
    }
    let from = path[0]
...
public func executeInputAction(
    ...
    case let .press(keyName, flags):
        guard let keyCode = mapKeyNameToKeyCode(keyName) else {
            throw ExactMacError.inputInvalidArgument("Unknown key name: \(keyName)")
        }
