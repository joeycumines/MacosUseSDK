...
ExactMac (server key `exactmac`) MCP server. Load this file when you need step-by-step procedures
...
```
wait(duration)    — pause for the given duration in seconds (max EXACTMAC_REQUEST_TIMEOUT; default 1)
```
