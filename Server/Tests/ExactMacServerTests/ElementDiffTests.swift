...
final class ElementDiffTests: XCTestCase {
    ...
    func testRoleChangeDetected() {
        let old = Exactmac_V1_Element.with { $0.role = "button" }
        let new = Exactmac_V1_Element.with { $0.role = "checkbox" }
    ...
    func testElementPathKeyEmptyPathsWithDifferentPositions() {
        ...
        let key1 = ExactMacService.elementPathKey(element1)
        let key2 = ExactMacService.elementPathKey(element2)
