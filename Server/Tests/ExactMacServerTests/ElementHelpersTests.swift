...
/// Unit tests for ElementHelpers extension methods on ExactMacService.
///
...
final class ElementHelpersTests: XCTestCase {
    ...
    func testElementMatchesCondition_attribute_keyExistsWithCorrectValue_returnsTrue() async {
        ...
        var condition = Exactmac_V1_StateCondition()
        var attrCond = Exactmac_V1_AttributeCondition()
    ...
    func testElementMatchesCondition_none_alwaysReturnsTrue() async {
        let service = await makeService()
        let element = makeElement()
        let condition = Exactmac_V1_StateCondition() // Default is .none
