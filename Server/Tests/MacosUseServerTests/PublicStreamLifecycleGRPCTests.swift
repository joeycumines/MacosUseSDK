...
struct PublicStreamLifecycleGRPCTests {
    ...
    func `client disconnect cancels both advertised stream producers`() async throws {
        ...
        try await withPublicStreamClient(composition) { client, disconnect in
            ...
            let observation = Task {
                try await publicServerStreaming(
                    client: client,
                    request: Macosusesdk_V1_StreamObservationsRequest.with { $0.name = observationName },
                    descriptor: Macosusesdk_V1_MacosUse.Method.StreamObservations.descriptor,
                    responseType: Macosusesdk_V1_StreamObservationsResponse.self,
                ) { response in
                    try await response.messages.reduce(into: 0) { count, _ in count += 1 }
                }
            }
    ...
    func `watch caller cancellation and service drain join the blocked producer`() async throws {
        ...
        let drainComposition = MacosUseServiceComposition(
            system: system,
            legacyPIDResourceNamesForTests: true,
    ...
    func `observation stream consumes disconnects and removes its continuation`() async throws {
        ...
        _ = try await composition.observationManager.createObservation(
            ...
            type: .elementChanges,
            parent: "applications/111",
            filter: Macosusesdk_V1_ObservationFilter.with { $0.pollInterval = 0.1 },
            pid: 111,
        )
        ...
        try await withPublicStreamClient(composition) { client, _ in
            ...
            let stream = Task {
                try await publicServerStreaming(
                    client: client,
                    request: Macosusesdk_V1_StreamObservationsRequest.with { $0.name = name },
                    descriptor: Macosusesdk_V1_MacosUse.Method.StreamObservations.descriptor,
                    responseType: Macosusesdk_V1_StreamObservationsResponse.self,
                ) { response in
    ...
    func `observation caller cancellation producer failure and drain release every owner`() async throws {
        ...
        try await withPublicStreamClient(composition) { client, _ in
            ...
            let cancelledStream = Task {
                try await publicServerStreaming(
                    client: client,
                    request: Macosusesdk_V1_StreamObservationsRequest.with { $0.name = cancelName },
                    descriptor: Macosusesdk_V1_MacosUse.Method.StreamObservations.descriptor,
                    responseType: Macosusesdk_V1_StreamObservationsResponse.self,
                ) { response in
                    try await response.messages.reduce(into: 0) { count, _ in count += 1 }
            ...
            let failedStream = Task {
                try await publicServerStreaming(
                    client: client,
                    request: Macosusesdk_V1_StreamObservationsRequest.with { $0.name = failureName },
                    descriptor: Macosusesdk_V1_MacosUse.Method.StreamObservations.descriptor,
                    responseType: Macosusesdk_V1_StreamObservationsResponse.self,
                ) { response in
                    try await response.messages.reduce(into: 0) { count, _ in count += 1 }
            ...
            let drainedStream = Task {
                try await publicServerStreaming(
                    client: client,
                    request: Macosusesdk_V1_StreamObservationsRequest.with { $0.name = drainName },
                    descriptor: Macosusesdk_V1_MacosUse.Method.StreamObservations.descriptor,
                    responseType: Macosusesdk_V1_StreamObservationsResponse.self,
                ) { response in
                    try await response.messages.reduce(into: 0) { count, _ in count += 1 }
...
private func withPublicStreamClient(
    ...
    let socketPath = "/tmp/macosuse-func003-\(UUID().uuidString).sock"
