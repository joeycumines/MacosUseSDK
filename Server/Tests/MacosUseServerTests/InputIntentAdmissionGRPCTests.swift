...
struct InputIntentAdmissionGRPCTests {
    ...
    func `target and output-only intent reject before state identity history or sinks`() async throws {
        ...
        let probes: [(
            label: String,
            mutate: @Sendable (inout Macosusesdk_V1_CreateInputRequest) -> Void,
        )] = [
            (
                "absent target",
                { $0.input.clearTarget() },
            ...
            (
                ...
                { $0.input.target = Macosusesdk_V1_InputTarget() },
            ),
            (
            ...
            (
                ...
                {
                    $0.input.deliveryResult = Macosusesdk_V1_InputDeliveryResult.with {
        ...
        try await withInputIntentClient(composition) { client in
            for (index, probe) in probes.enumerated() {
                ...
                do {
                    let _: Macosusesdk_V1_Input = try await inputIntentUnary(
                        client: client,
                        request: request,
    ...
    func `contradictory and unsupported input intent rejects before state or sinks`() async throws {
        ...
        let invalidActions: [(String, Macosusesdk_V1_InputAction)] = [
            (
            ...
            (
                ...
                makeInputAction(showAnimation: true, animationDuration: 0.25) {
                    $0.typeText = Macosusesdk_V1_TextInput.with { $0.text = "must-not-type" }
                },
            ),
            (
                "held key visualization",
                makeInputAction(showAnimation: true, animationDuration: 0) {
                    $0.pressKey = Macosusesdk_V1_KeyPress.with {
                        $0.key = "return"
                        $0.holdDuration = 0.1
                    }
                },
            ...
            (
                ...
                makeInputAction(showAnimation: true, animationDuration: 0) {
                    $0.scroll = Macosusesdk_V1_Scroll.with { $0.vertical = 1 }
                },
            ),
            (
                "hover visualization",
                makeInputAction(showAnimation: true, animationDuration: 0) {
                    $0.hover = Macosusesdk_V1_Hover.with {
                        $0.position = Macosusesdk_Type_Point.with { $0.x = 1; $0.y = 2 }
                        $0.duration = 0.1
                    }
                },
            ),
            (
                ...
                makeInputAction(showAnimation: false, animationDuration: 0) {
                    $0.pressKey = Macosusesdk_V1_KeyPress.with { $0.key = "definitely-not-a-key" }
                },
            ),
        ]
    ...
    func `omitted defaults and supported animation intent reach the executor exactly`() async throws {
        ...
        try await withInputIntentClient(composition) { client in
            for (index, action) in actions.enumerated() {
                ...
                let response: Macosusesdk_V1_Input = try await inputIntentUnary(
                    client: client,
                    request: request,
                    descriptor: Macosusesdk_V1_MacosUse.Method.CreateInput.descriptor,
                )
...
private struct CapturedInputIntent: Sendable {
    let action: MacosUseSDK.InputAction
}

private actor InputIntentRecorder {
    private var entries: [CapturedInputIntent] = []

    func record(action: MacosUseSDK.InputAction) {
        entries.append(
            CapturedInputIntent(
                action: action,
            ),
...
private func makeInputAction(
    ...
    configure: (inout Macosusesdk_V1_InputAction) -> Void,
) -> Macosusesdk_V1_InputAction {
...
private func makeClick() -> Macosusesdk_V1_MouseClick {
...
private func makeDrag() -> Macosusesdk_V1_MouseDrag {
    Macosusesdk_V1_MouseDrag.with {
        $0.startPosition = Macosusesdk_Type_Point.with { $0.x = 1; $0.y = 2 }
        $0.endPosition = Macosusesdk_Type_Point.with { $0.x = 3; $0.y = 4 }
    }
}
