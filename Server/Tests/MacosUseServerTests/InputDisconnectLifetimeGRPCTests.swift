...
struct InputDisconnectLifetimeGRPCTests {
    ...
    func `abrupt production disconnect retains cleanup through service drain`() async throws {
        ...
        try await withInputDisconnectClients(composition) { victim, control, disconnectVictim in
            ...
            let victimCall = Task {
                do {
                    let _: Macosusesdk_V1_Input = try await inputDisconnectUnary(
                        client: victim,
...
private func inputDisconnectCreateRequest() -> Macosusesdk_V1_CreateInputRequest {
...
private func prewarmInputDisconnectControlClient(
    ...
    do {
        let _: Macosusesdk_V1_Input = try await getInputThroughControlClient(
            client,
...
private func withInputDisconnectClients(
    ...
    let socketPath = "/tmp/macosuse-func004-input-disconnect-\(UUID().uuidString).sock"
