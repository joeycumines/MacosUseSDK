...
struct WindowCaptureTruthContractGRPCTests {
    ...
    func `generated window capture forwards exact source and reports captured metadata`() async throws {
        ...
        try await withWindowCaptureTruthClient(fixture.composition) { client in
            ...
            let response: Macosusesdk_V1_CaptureWindowScreenshotResponse = try await windowCaptureTruthUnary(
                client: client,
        ...
        for (includeOCR, result) in invalidResults {
            ...
            try await withWindowCaptureTruthClient(fixture.composition) { client in
                ...
                await expectWindowCaptureTruthRPCError(.unavailable) {
                    let _: Macosusesdk_V1_CaptureWindowScreenshotResponse = try await windowCaptureTruthUnary(
                        client: client,
                        request: Macosusesdk_V1_CaptureWindowScreenshotRequest.with {
                            $0.window = windowName
                            $0.includeOcrText = includeOCR
        ...
        try await withWindowCaptureTruthClient(fixture.composition) { client in
            ...
            let capture = Task {
                try await windowCaptureTruthUnary(
                    ...
                    descriptor: Macosusesdk_V1_MacosUse.Method.CaptureWindowScreenshot.descriptor,
                ) as Macosusesdk_V1_CaptureWindowScreenshotResponse
            }
...
private func withWindowCaptureTruthClient(
    ...
    let server = GRPCServer(transport: transport.server, services: [composition.macosUseService])
...
private func listWindowCaptureTruthName(
    ...
    let response: Macosusesdk_V1_ListWindowsResponse = try await windowCaptureTruthUnary(
        client: client,
...
private func windowCaptureTruthRegion(_ frame: CGRect) -> Macosusesdk_Type_Region {
    .with {
        $0.x = frame.origin.x
        $0.y = frame.origin.y
        $0.width = frame.width
