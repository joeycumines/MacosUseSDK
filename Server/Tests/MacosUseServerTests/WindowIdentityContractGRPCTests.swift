...
struct WindowIdentityContractGRPCTests {
    ...
    func `ListWindows deterministically enumerates one owner's synthetic multiwindow snapshot`(
        ...
        try await withWindowIdentityClient(fixture.composition) { client in
            let first: Macosusesdk_V1_ListWindowsResponse = try await windowIdentityUnary(
                client: client,
            ...
            let second: Macosusesdk_V1_ListWindowsResponse = try await windowIdentityUnary(
                client: client,
                request: Macosusesdk_V1_ListWindowsRequest.with {
                    $0.parent = fixture.applicationName
                    $0.orderBy = "layer"
    ...
    func `ListWindows continuation remains bound to its immutable snapshot`() async throws {
        ...
        try await withWindowIdentityClient(fixture.composition) { client in
            ...
            while !token.isEmpty {
                let page: Macosusesdk_V1_ListWindowsResponse = try await windowIdentityUnary(
                    client: client,
                    request: Macosusesdk_V1_ListWindowsRequest.with {
                        $0.parent = fixture.applicationName
                        $0.pageSize = 1
    ...
    func `owner replacement during the CG snapshot publishes no windows`() async throws {
        ...
        try await withWindowIdentityClient(fixture.composition) { client in
            await expectWindowRPCError(.notFound) {
                let _: Macosusesdk_V1_ListWindowsResponse = try await windowIdentityUnary(
                    client: client,
                    request: Macosusesdk_V1_ListWindowsRequest.with { $0.parent = fixture.applicationName },
                    descriptor: Macosusesdk_V1_MacosUse.Method.ListWindows.descriptor,
                )
            }
        }
    ...
    func `missing private window ID never enters heuristic selection`() async throws {
        ...
        try await withWindowIdentityClient(fixture.composition) { client in
            ...
            await expectWindowRPCError(.unavailable) {
                let _: Macosusesdk_V1_Window = try await windowIdentityUnary(
                    client: client,
    ...
    func `random or cross-owner window child cannot mutate the owner's sole window`(
        ...
        try await withWindowIdentityClient(fixture.composition) { client in
            await expectWindowRPCError(.notFound) {
                let _: Macosusesdk_V1_Window = try await windowIdentityUnary(
                    client: client,
                    request: Macosusesdk_V1_FocusWindowRequest.with { $0.name = forgedName },
    ...
    func `observed removal retires a name before the same CG ID is reused`() async throws {
        ...
        try await withWindowIdentityClient(fixture.composition) { client in
            ...
            await expectWindowRPCError(.notFound) {
                let _: Macosusesdk_V1_Window = try await windowIdentityUnary(
                    client: client,
                    request: Macosusesdk_V1_GetWindowRequest.with { $0.name = oldName },
                    descriptor: Macosusesdk_V1_MacosUse.Method.GetWindow.descriptor,
                )
            }
        }
    ...
    func `admitted AX element preserves its public name when its CG ID changes`() async throws {
        ...
        try await withWindowIdentityClient(fixture.composition) { client in
            ...
            let moved: Macosusesdk_V1_Window = try await windowIdentityUnary(
                client: client,
            ...
            let fetched: Macosusesdk_V1_Window = try await windowIdentityUnary(
                client: client,
                request: Macosusesdk_V1_GetWindowRequest.with { $0.name = stableName },
                descriptor: Macosusesdk_V1_MacosUse.Method.GetWindow.descriptor,
            )
    ...
    func `cancelling exact lookup releases the mutation without a late AX sink`() async throws {
        ...
        let listed = try await fixture.composition.macosUseService.listWindows(
            ...
            context: windowServerContext(Macosusesdk_V1_MacosUse.Method.ListWindows.descriptor),
        )
        let name = try #require(listed.message.windows.first?.name)
        fixture.system.failExactWindowIDLookups()

        let task = Task {
            try await fixture.composition.macosUseService.focusWindow(
                request: ServerRequest(
                    metadata: Metadata(),
                    message: Macosusesdk_V1_FocusWindowRequest.with { $0.name = name },
                ),
                context: windowServerContext(Macosusesdk_V1_MacosUse.Method.FocusWindow.descriptor),
            )
        }
...
private func makeWindowIdentityFixture(
    ...
    let composition = MacosUseServiceComposition(stateStore: stateStore, system: system)
