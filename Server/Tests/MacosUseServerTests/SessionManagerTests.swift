...
final class SessionManagerTests: XCTestCase {
    ...
    func testCreateSessionWithProvidedId() async throws {
    ...
    func testCreateSessionRejectsDuplicateResourceName() async throws {
        let manager = SessionManager()
        ...
        do {
            ...
            XCTFail("Expected duplicate session creation to fail")
        } catch let error as SessionError {
            XCTAssertEqual(error, .alreadyExists)
        }

        let stored = await manager.getSession(name: "sessions/duplicate-session")
        XCTAssertEqual(stored?.displayName, "Original")
    }

    func testCreateSessionWithAutoGeneratedId() async throws {
        let manager = SessionManager()
    ...
    func testCreateSessionWithMetadata() async throws {
        let manager = SessionManager()
    ...
    func testDeleteSessionReturnsTrueAndRemoves() async throws {
        let manager = SessionManager()
    ...
    func testListSessionsEmptyResult() async throws {
        ...
        let testSession = try await manager.createSession(
        ...
        let (sessions, _) = try await manager.listSessions(pageSize: 10, pageToken: nil)
        XCTAssertTrue(sessions.allSatisfy { $0.name.hasPrefix("sessions/") })
    }

    func testListSessionsPagination() async throws {
        let manager = SessionManager()
        ...
        let (firstPage, _) = try await manager.listSessions(pageSize: 2, pageToken: nil)
    ...
    func testBeginTransactionUpdatesSessionState() async throws {
        ...
        let (transactionId, _, updatedSession) = try await manager.beginTransaction(
    ...
    func testBeginTransactionTwiceThrows() async throws {
        ...
        let (transactionId, _, _) = try await manager.beginTransaction(
            sessionName: session.name,
            isolationLevel: .serializable,
            timeout: 60,
        )
    ...
    func testCommitTransactionRestoresActiveState() async throws {
        ...
        let (txId, _, _) = try await manager.beginTransaction(
            sessionName: session.name,
            isolationLevel: .serializable,
            timeout: 60,
        )
    ...
    func testAddDuplicateApplicationIsIdempotent() async throws {
        let manager = SessionManager()
    ...
    func testRemoveApplicationFromSession() async throws {
        let manager = SessionManager()
    ...
    func testAddObservationToSession() async throws {
        let manager = SessionManager()
    ...
    func testRemoveObservationFromSession() async throws {
        let manager = SessionManager()
    ...
    func testSessionHasExpireTime() async throws {
        let manager = SessionManager()
    ...
    func testSessionExpireTimeIs1HourFromCreate() async throws {
        let manager = SessionManager()
    ...
    func testConcurrentSessionCreation() async throws {
        ...
        try await withThrowingTaskGroup(of: Macosusesdk_V1_Session.self) { group in
            for i in 0 ..< count {
                group.addTask {
    ...
    func testConcurrentSessionDeletion() async throws {
        let manager = SessionManager()
        var createdNames: [String] = []
    ...
    func testInvalidateAllSessionsReturnsCount() async throws {
        let manager = SessionManager()

        // Create a few sessions
        var createdNames: [String] = []
        for i in 0 ..< 3 {
            let s = try await manager.createSession(
    ...
    func testInvalidateAllSessionsIsIdempotent() async throws {
        let manager = SessionManager()

        // Create and invalidate
        let s = try await manager.createSession(sessionId: "test-idempotent-invalidate", displayName: "Test", metadata: [:])
