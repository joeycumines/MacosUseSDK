...
final class ElementInputPublisherC6Tests: XCTestCase {
    ...
    func testC6_ClickPublishesRetrievableCompletedInput() async throws {
        ...
        let ownerID = UUID()
        let receipt = MacosUseSDK.InputExecutionReceipt(
            route: .session,
            postedEventCount: 2,
        ...
        XCTAssertEqual(published.state, Macosusesdk_V1_Input.State.completed, "successful delivery must finish COMPLETED")
        ...
        if case let .click(click) = published.action.inputType {
            XCTAssertEqual(click.clickType, Macosusesdk_V1_MouseClick.ClickType.left)
    ...
    func testC6_UnsupportedActionShapeDeliversButPublishesNoResource() async throws {
        ...
        let name = try await service.publishElementInputResource(
            ...
            executor: {
                executorInvoked = true
                return MacosUseSDK.InputExecutionReceipt(
                    route: .session,
                    postedEventCount: 1,
                    routedDeliveryObserved: true,
                )
