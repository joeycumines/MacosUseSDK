...
struct DisplayCaptureTruthContractGRPCTests {
    ...
    func `ListDisplays returns one exact immutable topology snapshot`() async throws {
        ...
        try await withDisplayCaptureTruthClient(composition) { client in
            let response: Macosusesdk_V1_ListDisplaysResponse = try await displayCaptureTruthUnary(
                client: client,
    ...
    func `GetDisplay rejects an inactive parseable display without consulting capture`() async throws {
        ...
        try await withDisplayCaptureTruthClient(composition) { client in
            await expectDisplayCaptureTruthRPCError(.notFound) {
                let _: Macosusesdk_V1_Display = try await displayCaptureTruthUnary(
                    client: client,
                    request: Macosusesdk_V1_GetDisplayRequest.with { $0.name = "displays/999" },
    ...
    func `CaptureScreenshot selects the snapshot main or exact explicit display and reports actual metadata`(
        ...
        try await withDisplayCaptureTruthClient(composition) { client in
            let response: Macosusesdk_V1_CaptureScreenshotResponse = try await displayCaptureTruthUnary(
                client: client,
    ...
    func `CaptureRegionScreenshot selects the same negative-origin source explicitly or by inference`(
        ...
        try await withDisplayCaptureTruthClient(composition) { client in
            let response: Macosusesdk_V1_CaptureRegionScreenshotResponse = try await displayCaptureTruthUnary(
                client: client,
    ...
    func `CaptureRegionScreenshot rejects crossing and mirrored ambiguity before capture`() async throws {
        ...
        let crossingComposition = MacosUseServiceComposition(
        ...
        try await withDisplayCaptureTruthClient(crossingComposition) { client in
            await expectDisplayCaptureTruthRPCError(.invalidArgument) {
                let _: Macosusesdk_V1_CaptureRegionScreenshotResponse = try await displayCaptureTruthUnary(
                    client: client,
        ...
        let mirroredComposition = MacosUseServiceComposition(
    ...
    func `endpoint-overflowing region rejects before topology or capture ownership`() async throws {
        ...
        try await withDisplayCaptureTruthClient(composition) { client in
            await expectDisplayCaptureTruthRPCError(.invalidArgument) {
                let _: Macosusesdk_V1_CaptureRegionScreenshotResponse =
    ...
    func `capture-source topology mismatch fails unavailable before image capture`() async throws {
        ...
        try await withDisplayCaptureTruthClient(composition) { client in
            await expectDisplayCaptureTruthRPCError(.unavailable) {
                let _: Macosusesdk_V1_CaptureScreenshotResponse = try await displayCaptureTruthUnary(
                    client: client,
    ...
    func `CaptureCursorPosition resolves negative coordinates from the same snapshot`() async throws {
        ...
        try await withDisplayCaptureTruthClient(composition) { client in
            let response: Macosusesdk_V1_CaptureCursorPositionResponse = try await displayCaptureTruthUnary(
                client: client,
...
private actor DisplayCaptureTruthSink: ScreenshotCapturing {
    ...
    func captureWindow(
        ...
        format _: Macosusesdk_V1_ImageFormat,
        quality _: Int32,
...
private func displayCaptureTruthRegionRequest(
    _ region: CGRect,
) -> Macosusesdk_V1_CaptureRegionScreenshotRequest {
    .with {
...
private func displayCaptureTruthRegion(_ frame: CGRect) -> Macosusesdk_Type_Region {
    .with {
        $0.x = frame.origin.x
        $0.y = frame.origin.y
        $0.width = frame.width
