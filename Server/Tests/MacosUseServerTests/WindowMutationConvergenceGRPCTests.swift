...
struct WindowMutationConvergenceGRPCTests {
    ...
    func `focus waits for observed AX focus and raises the exact window`() async throws {
        ...
        try await withWindowMutationClient(fixture.composition) { client in
            ...
            let focused: Macosusesdk_V1_Window = try await windowMutationUnary(
                client: client,
                request: Macosusesdk_V1_FocusWindowRequest.with { $0.name = name },
                descriptor: Macosusesdk_V1_MacosUse.Method.FocusWindow.descriptor,
            )
    ...
    func `move accepts settled macOS clamping and returns the observed origin`() async throws {
        ...
        try await withWindowMutationClient(fixture.composition) { client in
            let name = try await listedWindowName(client: client, parent: fixture.applicationName)
            let moved: Macosusesdk_V1_Window = try await windowMutationUnary(
                client: client,
    ...
    func `resize accepts settled macOS clamping and returns the observed size`() async throws {
        ...
        try await withWindowMutationClient(fixture.composition) { client in
            let name = try await listedWindowName(client: client, parent: fixture.applicationName)
            let resized: Macosusesdk_V1_Window = try await windowMutationUnary(
                client: client,
                request: Macosusesdk_V1_ResizeWindowRequest.with {
                    $0.name = name
                    $0.width = 20000
                    $0.height = 20000
                },
                descriptor: Macosusesdk_V1_MacosUse.Method.ResizeWindow.descriptor,
            )
    ...
    func `minimize and restore wait for the requested observed state`(restore: Bool) async throws {
        ...
        try await withWindowMutationClient(fixture.composition) { client in
            let name = try await listedWindowName(client: client, parent: fixture.applicationName)
            let response: Macosusesdk_V1_Window = if restore {
                try await windowMutationUnary(
                    client: client,
                    request: Macosusesdk_V1_RestoreWindowRequest.with { $0.name = name },
                    descriptor: Macosusesdk_V1_MacosUse.Method.RestoreWindow.descriptor,
                )
            } else {
                try await windowMutationUnary(
                    client: client,
                    request: Macosusesdk_V1_MinimizeWindowRequest.with { $0.name = name },
                    descriptor: Macosusesdk_V1_MacosUse.Method.MinimizeWindow.descriptor,
                )
            }
    ...
    func `close reports success only after exact AX disappearance and retires the binding`() async throws {
        ...
        try await withWindowMutationClient(fixture.composition) { client in
            let name = try await listedWindowName(client: client, parent: fixture.applicationName)
            let closed: Macosusesdk_V1_CloseWindowResponse = try await windowMutationUnary(
                client: client,
                request: Macosusesdk_V1_CloseWindowRequest.with { $0.name = name },
            ...
            await expectWindowMutationRPCError(.notFound) {
                let _: Macosusesdk_V1_Window = try await windowMutationUnary(
                    client: client,
    ...
    func `unreadable sibling AX identity fails before close side effects`() async throws {
        ...
        try await withWindowMutationClient(fixture.composition) { client in
            ...
            await expectWindowMutationRPCError(.unavailable) {
                let _: Macosusesdk_V1_CloseWindowResponse = try await windowMutationUnary(
                    client: client,
                    request: Macosusesdk_V1_CloseWindowRequest.with { $0.name = name },
                    descriptor: Macosusesdk_V1_MacosUse.Method.CloseWindow.descriptor,
                )
    ...
    func `caller cancellation stops convergence polling and releases the mutation gate`() async throws {
        ...
        let listed = try await fixture.composition.macosUseService.listWindows(
            request: ServerRequest(
                metadata: Metadata(),
                message: Macosusesdk_V1_ListWindowsRequest.with { $0.parent = fixture.applicationName },
            ),
            context: windowMutationServerContext(Macosusesdk_V1_MacosUse.Method.ListWindows.descriptor),
        )
        let name = try #require(listed.message.windows.first?.name)
        let baselineReads = fixture.system.snapshot().positionReads

        let task = Task {
            try await fixture.composition.macosUseService.moveWindow(
                request: ServerRequest(
                    metadata: Metadata(),
                    message: Macosusesdk_V1_MoveWindowRequest.with {
                        $0.name = name
                        $0.x = 900
                        $0.y = 700
                    },
                ),
                context: windowMutationServerContext(Macosusesdk_V1_MacosUse.Method.MoveWindow.descriptor),
            )
        }
...
private func withWindowMutationClient(
    ...
    let server = GRPCServer(transport: inProcess.server, services: [composition.macosUseService])
...
private func listedWindowName(
    ...
    let response: Macosusesdk_V1_ListWindowsResponse = try await windowMutationUnary(
        client: client,
