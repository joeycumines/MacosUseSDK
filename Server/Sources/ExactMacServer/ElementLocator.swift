...
private let logger = ExactMac.sdkLogger(category: "ElementLocator")
...
public actor ElementLocator {
    ...
    public func findElements(
        selector: Exactmac_Type_ElementSelector,
        parent: String,
        visibleOnly: Bool = false,
        maxResults: Int = 0,
    ) async throws -> [(element: Exactmac_V1_Element, path: [Int32])] {
    ...
    public func getElement(name: String) async throws -> Exactmac_V1_Element {
    ...
    private nonisolated static func executeSDKTraversal(
        ...
        let task = Task.detached(priority: .userInitiated) {
            let response = if let root {
                try ExactMac.traverseAccessibilitySubtree(
                    pid: pid,
                    rootElement: root,
                    onlyVisibleElements: visibleOnly,
                )
            } else {
                try ExactMac.traverseAccessibilityTree(
                    pid: pid,
                    onlyVisibleElements: visibleOnly,
    ...
    func matchesSelector(
        _ element: Exactmac_V1_Element, selector: Exactmac_Type_ElementSelector,
    ) -> Bool {
    ...
    private func isElementInRegion(
        _ element: Exactmac_V1_Element, region: Exactmac_Type_Region,
    )
        -> Bool
    {
