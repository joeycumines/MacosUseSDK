...
public actor MacroRegistry {
    private let logger = Logger(subsystem: "ExactMacServer", category: "MacroRegistry")
    private var macros: [String: Exactmac_V1_Macro] = [:]
    private let persistenceURL: URL?

    public init() {
        // Default persistence location: Application Support/ExactMacServer/macros.json
        if let appSupport = FileManager.default.urls(
            for: .applicationSupportDirectory,
            in: .userDomainMask,
        ).first {
            let serverDir = appSupport.appendingPathComponent("ExactMacServer", isDirectory: true)
    ...
    public func createMacro(
        ...
        parameters: [Exactmac_V1_MacroParameter],
        tags: [String],
    ) -> Exactmac_V1_Macro {
    ...
    public func getMacro(name: String) -> Exactmac_V1_Macro? {
        macros[name]
    }
    ...
    public func load(clearExisting: Bool = true) throws(PersistenceError) {
        ...
        for jsonString in store.macros {
            ...
            do {
                let macro = try Exactmac_V1_Macro(jsonUTF8Data: jsonData)
