...
public struct ServerState: Sendable {
    ...
    public var applications: [pid_t: Exactmac_V1_Application] = [:]
    /// Map of input name to Input proto message
    public var inputs: [String: Exactmac_V1_Input] = [:]
}
...
public actor AppStateStore {
    ...
    enum InputTerminalOutcome: Sendable {
        case completed(Exactmac_V1_InputDeliveryResult)
        case failed(error: String, delivery: Exactmac_V1_InputDeliveryResult)
        case cancelled(error: String, delivery: Exactmac_V1_InputDeliveryResult)
    }

    enum InputFinishResult: Sendable {
        case finished(Exactmac_V1_Input)
        case leaseLost
        case missingPublishedState
        case alreadyTerminal(Exactmac_V1_Input)
    }
    ...
    private var inputStateHistories: [String: [Exactmac_V1_Input.State]] = [:]
    ...
    public func addTarget(
        _ target: Exactmac_V1_Application,
    ...
    public func replaceTargets(
        _ targets: [(application: Exactmac_V1_Application, identity: ApplicationProcessIdentity)],
    ) {
    ...
    public func reconcileTargets(
        _ discoveredTargets: [(application: Exactmac_V1_Application, identity: ApplicationProcessIdentity)],
    ...
    public func removeTarget(pid: pid_t) -> Exactmac_V1_Application? {
    ...
    public func removeTarget(name: String) -> Exactmac_V1_Application? {
    ...
    public func getTarget(pid: pid_t) -> Exactmac_V1_Application? {
        state.applications[pid]
    }

    /// Gets only the exact current process instance named by the caller.
    public func getTarget(name: String) -> Exactmac_V1_Application? {
        guard let pid = applicationPIDsByName[name],
              state.applications[pid]?.name == name
        else {
            return nil
    ...
    public func listTargets() -> [Exactmac_V1_Application] {
    ...
    func publishPendingInput(
        ...
        let input = Exactmac_V1_Input.with {
            $0.name = lease.name
            $0.action = action
            $0.target = target
            $0.state = .pending
    ...
    func seedInputForTesting(_ input: Exactmac_V1_Input) {
    ...
    private func normalizedInputTerminalRecord(
        currentState: Exactmac_V1_Input.State,
        outcome: InputTerminalOutcome,
    ) -> (
        state: Exactmac_V1_Input.State,
        error: String,
        delivery: Exactmac_V1_InputDeliveryResult,
    ) {
        switch outcome {
    ...
    func inputStateHistory(name: String) -> [Exactmac_V1_Input.State] {
    ...
    public func getInput(name: String) -> Exactmac_V1_Input? {
        state.inputs[name]
    }

    /// Lists inputs for a parent
    public func listInputs(parent: String) -> [Exactmac_V1_Input] {
