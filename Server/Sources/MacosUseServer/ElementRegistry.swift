...
struct RegisteredElementMutationTarget: @unchecked Sendable {
    ...
    let element: Macosusesdk_V1_Element
...
public actor ElementRegistry {
    ...
    public func registerElement(
        _ element: Macosusesdk_V1_Element,
    ...
    private static func makeProtoElement(
        from elementData: MacosUseSDK.ElementData,
    ...
    public func getElement(_ elementId: String) -> Macosusesdk_V1_Element? {
    ...
    func listElements(forPID pid: pid_t) -> [Macosusesdk_V1_Element] {
        let now = clock()
    ...
    func getElement(_ elementId: String, expectedPID: pid_t) -> Macosusesdk_V1_Element? {
        guard let cached = elementCache[elementId] else {
            logger.warning("Element \(elementId, privacy: .private) not found in cache")
            return nil
        }
