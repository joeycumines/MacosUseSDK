...
actor SessionManager {
  ...
  func listSessions(pageSize: Int, pageToken: String?) async -> (
    ...
    let allSessions = sessions.values.map { $0.session }.sorted { $0.name < $1.name }

    // Simple pagination: page token is the last session name
    let startIndex: Int
    if let token = pageToken, !token.isEmpty {
      startIndex = allSessions.firstIndex(where: { $0.name > token }) ?? allSessions.count
    } else {
      startIndex = 0
    }
  ...
  func commitTransaction(sessionName: String, transactionId: String) async throws -> (
    session: Macosusesdk_V1_Session,
    transaction: Macosusesdk_V1_Transaction,
    ...
    return (state.session, committedTransaction, operationsCount)
  }
  ...
  func rollbackTransaction(
    ...
    return (state.session, rolledBackTransaction, Int32(rolledBackCount))
  }
...
enum SessionError: Error, CustomStringConvertible {
  ...
  var description: String {
    ...
    case .transactionAlreadyActive:
      return "Transaction already active for this session"
    case .noActiveTransaction:
      return "No active transaction for this session"
    case .transactionMismatch:
      return "Transaction ID does not match active transaction"
    case .invalidSessionState:
      return "Session is not in a valid state for this operation"
    case .revisionNotFound:
      return "Revision ID not found"
    }
  }
}
