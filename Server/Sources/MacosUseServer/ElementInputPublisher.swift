...
// OUTPUT_ONLY resource_reference fields to a retrievable macosusesdk.com/Input.
...
extension MacosUseService {
    ...
    func publishElementInputResource(
        ...
        parent: String,
        ownerID: UUID,
        route _: MacosUseSDK.InputDeliveryRoute,
        executor: @MainActor () async throws -> MacosUseSDK.InputExecutionReceipt,
    ) async throws -> String {
        ...
        do {
            ...
            let evidence: (postedEventCount: Int, routedDeliveryObserved: Bool) = if let failure = error as? MacosUseSDK.InputExecutionFailure {
                (
            ...
            let isCancellation = error is CancellationError
                || (error as? MacosUseSDK.InputExecutionFailure).map {
    ...
    private static func elementInputActionProto(
        from action: MacosUseSDK.InputAction,
    ...
    private static func mouseClick(
        point: CGPoint,
        button: Macosusesdk_V1_MouseClick.ClickType,
        clickCount: Int,
        modifiers: [Macosusesdk_V1_KeyPress.Modifier] = [],
    ) -> Macosusesdk_V1_MouseClick {
        .with {
            $0.position = .with {
                $0.x = point.x
                $0.y = point.y
    ...
    ) -> Macosusesdk_V1_MouseClick.ClickType {
        switch button {
    ...
    ) -> [Macosusesdk_V1_KeyPress.Modifier] {
        var out: [Macosusesdk_V1_KeyPress.Modifier] = []
