...
enum ScreenshotError: Error, CustomStringConvertible {
  ...
  var description: String {
    ...
    case .captureFailedWindow(let windowID):
      return "Failed to capture window \(windowID)"
    case .captureFailedRegion(let bounds):
      return "Failed to capture region \(bounds)"
    case .invalidRegion:
      return "Invalid region bounds (width/height must be > 0)"
    case .encodingFailed(let format):
      return "Failed to encode image in format \(format)"
    case .windowNotFound(let windowID):
      return "Window \(windowID) not found"
