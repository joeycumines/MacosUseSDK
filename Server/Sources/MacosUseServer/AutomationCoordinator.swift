...
private let unscopedInputExecutionBoundary = MacosUseSDK.InputExecutionBoundary(
    validateEffect: { _ in },
...
typealias InputActionExecutor = @MainActor @Sendable (
    ...
    MacosUseSDK.InputDeliveryRoute,
    MacosUseSDK.InputExecutionBoundary,
) async throws -> MacosUseSDK.InputExecutionReceipt

typealias InputPostAccessChecker = @Sendable () -> Bool
typealias InputKeyResolver = @Sendable (String) -> MacosUseSDK.ResolvedInputKey?
typealias KeyboardInputSourceIdentityProvider =
    @Sendable () -> MacosUseSDK.KeyboardInputSourceIdentity?

typealias ApplicationOpenExecutor = @MainActor @Sendable (
    URL,
    Bool,
    MacosUseSDK.AppLaunchMode,
) async throws -> MacosUseSDK.AppOpenerResult

struct AccessibilityTraversalSnapshot: Sendable {
    let appName: String
    let elements: [MacosUseSDK.ElementData]
    let count: Int
    ...
    init(
        appName: String,
        elements: [MacosUseSDK.ElementData] = [],
        count: Int = 0,
        excludedCount: Int = 0,
    ...
    init(_ response: MacosUseSDK.ResponseData) {
        self.init(
...
struct PhysicalDesktopMutationContext: Sendable {
    ...
    @MainActor
    func executeInput(
        _ action: MacosUseSDK.InputAction,
        route: MacosUseSDK.InputDeliveryRoute,
        executionBoundary: MacosUseSDK.InputExecutionBoundary =
            unscopedInputExecutionBoundary,
        accessPreflighted: Bool = false,
        backendOperation: (@MainActor @Sendable (any MacosUseSDK.InputEventBackend) async throws -> Void)? = nil,
    ) async throws -> MacosUseSDK.InputExecutionReceipt {
        if !accessPreflighted {
            try Self.requireInputPostAccess(using: inputPostAccessChecker)
        }
        let receipt: MacosUseSDK.InputExecutionReceipt = if let inputActionExecutor {
...
public actor AutomationCoordinator {
    ...
    init(
        ...
        inputKeyResolver: @escaping InputKeyResolver = MacosUseSDK.resolveInputKey,
        keyboardInputSourceIdentityProvider: @escaping KeyboardInputSourceIdentityProvider =
            MacosUseSDK.currentKeyboardInputSourceIdentity,
        inputPostAccessChecker: @escaping InputPostAccessChecker = {
    ...
    public func handleOpenApplication(
        ...
        mode: MacosUseSDK.AppLaunchMode = .launchOrActivate,
    ) async throws -> MacosUseSDK.AppOpenerResult {
        ...
        if let applicationOpenExecutor {
            ...
            return try await MacosUseSDK.openApplication(
    ...
    nonisolated func validateInputAction(_ action: Macosusesdk_V1_InputAction) throws
    ...
    nonisolated func admitInputAction(
        ...
        let executionAction: MacosUseSDK.InputAction
        let sourceIdentity: MacosUseSDK.KeyboardInputSourceIdentity?
        ...
        case let .press(keyName, flags):
            ...
            guard MacosUseSDK.layoutIndependentKeyCode(for: keyName) != nil
    ...
    private nonisolated static func executeTraversal(
        ...
        let traversal: @Sendable () async throws -> Macosusesdk_V1_TraverseAccessibilityResponse = {
    ...
    private nonisolated static func mapTraversalError(_ error: any Error) -> any Error {
        ...
        if let sdkError = error as? MacosUseSDK.MacosUseSDKError {
    ...
    fileprivate static func executeInputAction(
        ...
        try await MacosUseSDK.executeObservedInputAction(
            action,
            route: route,
            executionBoundary: executionBoundary,
            backendOperation: backendOperation,
...
extension AutomationCoordinator {
        ...
        -> MacosUseSDK.InputAction
    {
        ...
        case let .pressKey(keyPress):
            ...
            guard MacosUseSDK.layoutIndependentKeyCode(for: keyPress.key) != nil
    ...
    private nonisolated func convertButtonType(
        _ buttonType: Macosusesdk_V1_MouseClick.ClickType,
        supplied: Bool,
    ) throws
        -> CGMouseButton
    {
