...
enum ClipboardPasteboardSnapshot: Equatable, Sendable {
    ...
    case canonical(Macosusesdk_V1_ClipboardContent?)
}
...
struct SystemClipboardPasteboard: @unchecked Sendable, ClipboardPasteboard {
    ...
    func read() async -> Macosusesdk_V1_Clipboard {
        ...
        let content: Macosusesdk_V1_ClipboardContent? = if let files, !files.isEmpty {
            Macosusesdk_V1_ClipboardContent.with {
                $0.type = .files
                $0.content = .files(
                    Macosusesdk_V1_FilePaths.with { $0.paths = files.map(\.path) },
                )
            }
        } else if let image {
            Macosusesdk_V1_ClipboardContent.with {
...
actor ClipboardManager {
    ...
    func readClipboard() async throws -> Macosusesdk_V1_Clipboard {
    ...
    func writeClipboard(content: Macosusesdk_V1_ClipboardContent) async throws
        -> Macosusesdk_V1_Clipboard
    {
        ...
        return try await mutationGate.withExclusiveOperation {
            try await self.accessGate.withExclusiveAccess {
                ...
                let observed: Macosusesdk_V1_Clipboard
                do {
    ...
    func clearClipboard() async throws -> Macosusesdk_V1_Clipboard {
        let pasteboard = self.pasteboard
        return try await mutationGate.withExclusiveOperation {
            try await self.accessGate.withExclusiveAccess {
                await pasteboard.clear()
    ...
    private nonisolated static func canonicalize(
        content: Macosusesdk_V1_ClipboardContent,
    ) throws -> Macosusesdk_V1_ClipboardContent {
        guard content.hasType else {
            throw ClipboardError.invalidContent("Content type is required")
        }
        let representedType: Macosusesdk_V1_ContentType
        let payload: Macosusesdk_V1_ClipboardContent.OneOf_Content
        switch content.content {
        case let .text(value):
        ...
        return Macosusesdk_V1_ClipboardContent.with {
    ...
    private nonisolated static func canonicalizeObservation(
        _ observed: Macosusesdk_V1_Clipboard,
        requireContent: Bool,
    ) throws -> Macosusesdk_V1_Clipboard {
    ...
    private nonisolated static func semanticallyEqual(
        _ lhs: Macosusesdk_V1_ClipboardContent,
        _ rhs: Macosusesdk_V1_ClipboardContent,
    ) -> Bool {
