...
actor ClipboardHistoryManager {
  ...
  func getHistory() -> Macosusesdk_V1_ClipboardHistory {
    return Macosusesdk_V1_ClipboardHistory.with {
      $0.entries = history
    }
  }
...
enum ClipboardError: Error, CustomStringConvertible {
  ...
  var description: String {
    ...
    case .invalidContent(let msg):
      return "Invalid clipboard content: \(msg)"
    case .writeFailed(let msg):
      return "Clipboard write failed: \(msg)"
    case .readFailed(let msg):
      return "Clipboard read failed: \(msg)"
    }
  }
}
