...
private struct InputExecutionTargetLease: Sendable {
    ...
    var deliveryRoute: MacosUseSDK.InputDeliveryRoute {
        switch owner {
        case let .application(application):
...
final class InputTransactionExecutor: Sendable {
    ...
    func execute(
        _ req: Macosusesdk_V1_CreateInputRequest,
        ...
        let terminalInput: Macosusesdk_V1_Input
        do {
            terminalInput = try await automationCoordinator.withOwnedMutationTask(
                ...
                { [self] runner, ownerID in
                    ...
                    var expectedRoute: MacosUseSDK.InputDeliveryRoute?
                    ...
                    do {
                        ...
                        let terminalState: Macosusesdk_V1_Input.State =
                        ...
                        let failedInput: Macosusesdk_V1_Input
                        switch await stateStore.finishInput(
                            lease: identityLease,
                            outcome: outcome,
                        ) {
    ...
    private func inputBackendOperation(
        ...
        -> (@MainActor @Sendable (any MacosUseSDK.InputEventBackend) async throws -> Void)?
    {
        let text: String
        ...
        return { backend in
            ...
            try await clipboardManager.withTemporaryText { replacePasteboardText in
                try await MacosUseSDK.writeTextByPasting(
                    text,
    ...
    private func validateInputTargetStructure(
        _ target: Macosusesdk_V1_InputTarget,
        parent: String,
    ) throws {
    ...
    private func inputActionPoints(_ action: MacosUseSDK.InputAction) -> [CGPoint] {
        switch action {
    ...
    private func validateInputPhysicalEffectAuthority(
        effect: MacosUseSDK.InputPhysicalEffect,
    ...
    private func inputExecutionFailureEvidence(
        _ error: any Error,
        expectedRoute: MacosUseSDK.InputDeliveryRoute?,
        committedEvidence: (
            postedEventCount: Int,
            routedDeliveryObserved: Bool,
        )?,
        ...
        if let failure = error as? MacosUseSDK.InputExecutionFailure {
    ...
    private func exactApplicationGenerationError(
        ...
        if error is MacosUseSDK.InputProcessRouteRetired {
...
func inputDeliveryResult(
    commitment: Macosusesdk_V1_InputDeliveryResult.Commitment,
    postedEventCount: Int32 = 0,
    routedDeliveryObserved: Bool = false,
) -> Macosusesdk_V1_InputDeliveryResult {
    Macosusesdk_V1_InputDeliveryResult.with {
...
extension MacosUseService {
    func createInput(
        request: ServerRequest<Macosusesdk_V1_CreateInputRequest>,
    ...
    private func parseInputStateFilter(
        _ rawFilter: String,
    ) throws -> Macosusesdk_V1_Input.State? {
...
private extension InputExecutionTargetLease {
    func validate(effect: MacosUseSDK.InputPhysicalEffect) throws {
    ...
    func validateProcessRoute(
        ...
        guard let expected, expected.pid == pid else {
            throw MacosUseSDK.InputProcessRouteRetired(pid: pid)
        }
...
private extension MacosUseSDK.InputPhysicalEffect {
