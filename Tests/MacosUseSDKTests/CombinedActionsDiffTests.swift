...
final class CombinedActionsDiffTests: XCTestCase {
    ...
    override func tearDown() async throws {
        ...
        fputs(
            "info: Test teardown - Waiting 1.5 seconds for app termination and UI settling...\n", stderr,
        )
    ...
    func testCalculatorMultiplyWithActionAndTraversalHighlight() async throws {
        ...
        let testActionHighlightDuration = 0.4
        let testTraversalHighlightDuration = 2.0 // Duration passed to SDK function
        let testDelayNano: UInt64 = 150_000_000

        // --- Action Sequence with Highlighting ---
        fputs(
            "info: Test run - Calling writeTextWithActionAndTraversalHighlight for '2*3='...\n", stderr,
        )
        ...
        fputs(
            "info: Test run - writeTextWithActionAndTraversalHighlight returned (highlighting may start appearing).\n",
            stderr,
        )
        ...
        if result.diff.added.isEmpty {
            ...
            for element in result.diff.added {
                fputs(
                    "info:   + Role: \(element.role), Text: \(element.text ?? "nil"), Pos: (\(element.x ?? -1), \(element.y ?? -1)), Size: (\(element.width ?? -1) x \(element.height ?? -1))\n",
                    stderr,
                )
            }
        }
        ...
        fputs(
            "info: Test run - Waiting \(highlightCompletionWaitSeconds) seconds for traversal highlighting animations to complete...\n",
            stderr,
        )
        try await Task.sleep(nanoseconds: UInt64(highlightCompletionWaitSeconds * 1_000_000_000))
        fputs(
            "info: Test run - Traversal highlight animation wait finished. Proceeding to finish test function.\n",
            stderr,
        )
        // --- END WAIT ---
