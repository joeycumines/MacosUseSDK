...
final class CombinedActionsFocusVisualizationTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        fputs(
            "info: Focus Test Setup - TextEdit launched with PID \(textEditPID!) opening \(fileURL.lastPathComponent)\n",
            stderr,
        )
    ...
    override func tearDown() async throws {
        fputs(
            "info: Focus Test Teardown - Terminating TextEdit (PID: \(textEditPID ?? -1)) and cleaning up file...\n",
            stderr,
        )
        ...
        if let pid = textEditPID {
            ...
            do {
                ...
                fputs(
                    "info: Focus Test Teardown - Attempted clean close via AppleScript (Status: \(process.terminationStatus))\n",
                    stderr,
                )
            } catch {
        ...
        if let fileURL = temporaryFileURL {
            do {
                ...
                fputs(
                    "info: Focus Test Teardown - Successfully deleted temporary file: \(fileURL.path)\n",
                    stderr,
                )
            } catch {
    ...
    func testTextEditFocusAndWriteVisualization() async throws {
        ...
        // --- Define durations ---
        let testActionHighlightDuration = 0.6
        let testTraversalHighlightDuration = 1.5 // Shorter for this test
        let testDelayNano: UInt64 = 200_000_000 // 0.2s
        let observationDelaySeconds = 1.0 // Time to observe action visualization
        ...
        fputs(
            "info: Test run - Check logs above for 'successfully found focused element center' from writeTextAndVisualize.\n",
            stderr,
        )
        ...
        fputs(
            "info: Test run - Waiting \(observationDelaySeconds) seconds for visual observation...\n",
            stderr,
        )
