...
The gRPC Swift 2 migration is complete with MacosUseServer building successfully (exit 0) with zero warnings; all service providers, proto stubs, and actors have been updated to grpc-swift-2 (GRPCCore + GRPCProtobuf + NIO HTTP/2 transport); remaining work focuses on pagination, observation features, and testing.

### **Immediate Action Items (Next Things To Do)**

1. **Restore Unix Domain Socket (UDS) support:** The gRPC server bootstrap logic in `Server/Sources/MacosUseServer/main.swift` must be corrected to handle the `unixSocketPath` configuration, restoring the ability to bind to a UDS instead of a TCP port.
2. **Correct macOS Deployment Target:** The deployment target in `Server/Package.swift` must be updated from `.macOS(.v12)` to `.macOS(.v15)` to align with `grpc-swift-2` requirements, and the `-disable-availability-checking` flag must be removed.
3. **Run comprehensive verification:** Execute `make all` to run linting, all unit tests, and integration tests to verify the migration hasn't broken existing functionality.
4. **Fix pagination to use opaque tokens everywhere:** AIP-158 requires `page_token` to be treated as an opaque string by clients. `listInputs` and `listWindows` currently use a readable `"offset:N"` format; keep any internal structure server-side only and update code/comments so tokens are generated and documented as opaque (no client should depend on their structure). Ensure all List/Find RPCs use pagination with opaque tokens.
5. **Paginate all List/Find RPCs consistently:** `ListApplications` and any other list RPCs that currently return unpaginated results must be updated to support `page_size`, `page_token`, and `next_page_token` with deterministic ordering. Remove the notion that it might be acceptable to leave some list RPCs unpaginated in v1 and align Phase 1/3 wording with the "everything paginated" requirement.
6. **Tighten `PerformElementAction` and related tests:** After confirming its actual coverage in `MacosUseServiceProvider.swift`, either extend it to the minimal, well-documented set of additional actions we care about for v1 or clearly constrain and test the limited set it supports today. Reflect this decision and the implemented set in Phase 3.2.
7. **Reconcile observation behavior with tests:** `ObservationManager` implements element and window change detection with polling, but current Go integration tests (`integration/main_test.go` and siblings) still rely on sleeps and basic List-based checks instead of a full PollUntil + delta pattern. Update Phase 4 to call out the concrete missing tests and add explicit tasks to replace sleeps with PollUntil and add delta assertions.
8. **Clarify golden app lifecycle guarantees vs actual harness:** The integration harness kills Calculator/TextEdit up front but does not yet implement per-test `DeleteApplication` RPC cleanup or full session/resource teardown. Reflect this gap in Phase 4.2 and add tasks for a proper fixture lifecycle aligned with the constraints.
