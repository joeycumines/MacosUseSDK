...
The gRPC Swift 2 migration and core server scaffolding are in place (including UDS support and ScreenCaptureKit-based screenshot capture); remaining work focuses on AIP-aligned pagination (with opaque tokens everywhere), selector/path semantics, observation diffs, and a PollUntil-based integration test matrix.

### **Immediate Action Items (Next Things To Do)**

1. **Run comprehensive verification:** Execute `make all` (via a dedicated `config.mk` target) to run linting, all unit tests, and integration tests, and treat any failure as evidence that plan items marked "designed" but not proven in tests are still open.
2. **Fix pagination semantics to match AIP‑158:** Several RPC implementations (`ListInputs`, `ListWindows`, `ListObservations`, `FindElements`, `FindRegionElements`) currently use a structured `"offset:N"` token; `ListApplications` ignores pagination fields entirely. Replace these with server-private, opaque tokens (e.g., last-key or encoded cursor) and update proto comments and tests so clients never depend on token structure.
3. **Paginate all List/Find RPCs consistently:** Implement pagination for `ListApplications` (currently returns the full list) and verify all other List/Find RPCs honour `page_size`, `page_token`, and `next_page_token` with deterministic ordering and consistent default/max page sizes.
4. **Align `PerformElementAction` with reality:** `PerformElementAction` currently supports a focused set of actions (press/click/showmenu/openmenu via AX or coordinate fallback) and returns `UNIMPLEMENTED` for others. Decide and document the v1 surface (stay narrow or add a small curated set), then add tests that cover both supported and unsupported actions. Keep Phase 3.2 aligned with this explicit scope.
5. **Make observations and element paths explicit in the plan:** `ObservationManager` implements polling-based element/window/attribute diffs keyed off the element `path`, but `ElementLocator` currently uses a simple index `[index]` as the path and carries a `FIXME` for hierarchical paths. Capture this as a concrete technical debt item (Phase 3/6) and ensure tests don’t assume more stable path semantics than the implementation provides.
6. **Reconcile PollUntil & golden app lifecycle with tests:** Integration tests already use `PollUntilContext` and cover pagination and input flows but still need to fully implement the state-delta pattern and aggressive golden-app cleanup described in Phase 4. Update Phase 4.2 and related sections to enumerate the missing tests and fixtures (per-test `DeleteApplication` RPC cleanup, per-suite app kill, and explicit delta assertions around window and element changes).
