...
1. **Eliminate every compiler warning listed by Hana:** Treat the current Swift build warning log as a blocker. Track each warning (invalid exclude path, deprecated `Path` usage in `GRPCSwiftPlugin`, `@retroactive` sendable conformance, forced `CFBoolean` cast, actor-isolated calls, unused bindings, redundant awaits) and resolve or explicitly silence them with justification so the server builds cleanly.
